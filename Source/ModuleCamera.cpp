...
#include "SDL.h"
#include "MathGeoLib_Source/Geometry/Math/float3x3.h"
...
void ModuleCamera::SetFOVrads(float rads)
{
	fovRads = rads;
...
void ModuleCamera::SetOrientation(float3 ori)
{
	frustum.SetUp(ori);
}
...
void ModuleCamera::SetLookAt(float3 lookAt)
{
	frustum.SetFront(lookAt);
}

void ModuleCamera::SetLookAt(float x, float y, float z)
{
	SetLookAt(float3(x, y, z));
}

void ModuleCamera::Translate(float3 deltaCoords)
{
...
void ModuleCamera::Translate(float deltaX, float deltaY, float deltaZ)
{
	Translate(float3(deltaX, deltaY, deltaZ));
}

void ModuleCamera::Rotate(float3 thetasRad) //TODO: not working properly with multiple rotations
{
...
void ModuleCamera::Rotate(float thetaXRad, float thetaYRad, float thetaZRad)
{
